...
!         write(*,*) 'Hello, I just started'
      ...
      integer function how_many_extra_history_columns(id)!JS says
         integer, intent(in) :: id
         integer :: ierr
         type (star_info), pointer :: s
         ierr = 0
...
!        write(*,*) 'I just took a step'

!        write(*,*) 'The central density is', s% log_center_density
!        write(*,*) 'The surface density is', s% rho(1)
...
!     Note: x_ctrl(1) is the mu_12 input parameter and is given from the inlist
         E_pl = sources(plas_neu_type,ineu) * 31.8d6 * (ev2erg*ev2erg) * (mu12*mu12) / (hbar*hbar * w_pl_2)
